...
def _merge_format_files(
	...
	filename (renamed on collision) or ``"<skipped:cover>"`` /
	``"<skipped:identical>"`` / ``"<skipped:generated-cover>"``. Metadata
	sidecars are NOT moved (subsumed by the merged metadata). In dry-run
	nothing is moved; the planned outcomes are still returned.

	A loser cover classified GENERATED by the C11 math is never moved: the
	gap-fill must not resurrect a placeholder a clean run had just bakked
	(measured 2026-09-16: a merge folded a duplicate back in as cover.jpg
	minutes after clean --generated bakked it). It dies with the loser
	folder like its sidecars.
	"""
	...
	for entry in sorted(loser_folder.iterdir(), key=lambda e: e.name):
		...
		if not (is_ebook or is_cover):
			continue
		if is_cover and analyze_cover(entry).is_generated:
			moved.append((entry.name, "<skipped:generated-cover>"))
			continue
