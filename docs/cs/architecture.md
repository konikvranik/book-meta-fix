...
| `llm.py` | Poskytovatel LLM Z.AI (kompatibilní s OpenAI) — opravce poslední instance. Vyhlazovač rychlosti `LeakyBucket` + globální 429 cooldown; `reconcile_loop` (Flash→feedback→final); tolerantní parsování JSON; `get_provider` = továrna providerů (`BMF_LLM_PROVIDER`: auto / zai / antigravity / mock / off). → `ReconciledMeta` |
| `acp.py` | `AntigravityAcpProvider` — předplatné Google Antigravity jako RYCHLÁ vrstva smyčky přes Agent Client Protocol (`BMF_ANTIGRAVITY_CMD`, oficiální `agy_acp_server.par`; funguje jakýkoli ACP agent). Jeden agentní proces na spojení, čerstvá relace na knihu, nástroje odmítány; transportní selhání se jednou opakují, tři za sebou rychlou vrstvu pro běh odloží; Z.AI (pokud je nastaveno) zůstává placenou zálohou. → `ReconciledMeta` |
