...
def strip_covers(library: Path | None, no_cache: bool, limit: int | None, do_apply: bool) -> None:
	"""Remove auto-generated (Calibre placeholder) covers from the whole library.

	Two targets per book: the cover.jpg sidecar classified as generated by the
	C11 pixel analysis (renamed to cover.jpg.bak, never hard-deleted) and the
	embedded cover of each EPUB that probes as generated (surgical zip+OPF
	strip). Non-EPUB format files are untouched — their covers live in binary
	EXTH headers with no safe removal path. After a write run the next
	...
	try:
		...
		with Progress(
			...
			task_id = progress.add_task(_("Stripping generated covers"), total=len(books))
			for meta in books:
				try:
					results.append(strip_generated_covers(meta.path, dry_run=not do_apply))
				except Exception as e:  # noqa: BLE001
